(library (tensorflow tensorflow)
  ...
  (define lib-name
    (case (machine-type)
      ((i3nt ti3nt a6nt ta6nt) "libtensorflow.dll")
      ((a6osx i3osx ta6osx ti3osx)  "libtensorflow.dylib")
      ((a6le i3le ta6le ti3le) "libtensorflow.so")
      (else "libtensorflow.so")))

  (define tensorflow-init
    (lambda (path)
      (load-shared-object path)))

  (define init (tensorflow-init lib-name))

  (define-syntax define-procedure
    (syntax-rules ()
      ((_ rst name args)
       (define name
          (let ([sym (symbol->string 'name)])
            (if (foreign-entry? sym)
              (foreign-procedure sym args rst)
              (lambda x (printf "error: ~a not found in ~a\n" sym lib-name))))))))

  (include "tensorflow/const.sc")
  (include "tensorflow/ftype.sc")
  
  ; // --------------------------------------------------------------------------
  ; // TF_Version returns a string describing version information of the
  ; // TensorFlow library. TensorFlow using semantic versioning.
  ; TF_CAPI_EXPORT extern const char* TF_Version();
  (define-procedure string TF_Version ())
    
  ; // TF_DataTypeSize returns the sizeof() for the underlying type corresponding
  ; // to the given TF_DataType enum value. Returns 0 for variable length types
  ; // (eg. TF_STRING) or on failure.
  ; TF_CAPI_EXPORT extern size_t TF_DataTypeSize(TF_DataType dt);
  (define-procedure size_t TF_DataTypeSize (int))

  ; // Return a new status object.
  ; TF_CAPI_EXPORT extern TF_Status* TF_NewStatus();
  (define-procedure (*TF_Status) TF_NewStatus ())

  ; // Delete a previously created status object.
  ; TF_CAPI_EXPORT extern void TF_DeleteStatus(TF_Status*);
  (define-procedure void TF_DeleteStatus ((* TF_Status))
